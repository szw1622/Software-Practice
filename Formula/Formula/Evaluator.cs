        ...
        public static Double Evaluate(String expression, Func<string, double> variableLookup)
        {
            ...
            for (int i = 0; i < substrings.Length; i++)
            {
                ...
                if (!sampleOperators.Contains(t))
                {
                    ...
                    if (Double.TryParse(t, out Double intT))
                    {
                        if (OperatorStack.Count != 0)
                        {
                            ...
                            if (OperatorStack.Peek().Equals("*"))
                            {
                                ...
                                try
                                {
                                    Double intPoped = Double.Parse(valPoped);
                            ...
                            else if (OperatorStack.Peek().Equals("/"))
                            {
                                ...
                                try
                                {
                                    ...
                                    //Push the result onto the value stack
                                    if (intT == 0)
                                        throw new DivideByZeroException("Can not divide " + valPoped + " by 0.");
                                    ValueStack.Push($"{intPoped / intT}");
                                    continue;
                                }
                                catch (FormatException)
                ...
                if ((t.Equals("+") || t.Equals("-")))
                {
                    if (OperatorStack.Count != 0)
                    {
                        ...
                        if (OperatorStack.Peek().Equals("+"))
                        {
                            ...
                            try
                            {
                                Double intTwo = Double.Parse(valTwo);
                                Double intOne = Double.Parse(valOne);
                                //push the result onto the value stack
                    ...
                    if (OperatorStack.Count != 0)
                    {
                        ...
                        else if (OperatorStack.Peek().Equals("/") && ValueStack.Count >= 2)
                        {
                            ...
                            try
                            {
                                ...
                                //Push the result onto the value stack
                                if (intTwo == 0)
                                    throw new DivideByZeroException("Can not divide " + valOne + " by 0.");
                                ValueStack.Push($"{intOne / intTwo}");
                                continue;
                            }
                            catch (FormatException)
            ...
            if (OperatorStack.Count == 0)
            {
                ...
                try
                {
                    return Double.Parse(popedResult);
                }
                catch (FormatException)
                {
